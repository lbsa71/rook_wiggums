...
- **5.1.2** ✅ Configure CSS styling solution (App.css, no Tailwind — single-page dashboard)
- **5.1.3** ✅ Set up Vite proxy for `/api` and `/ws` to backend
- **5.1.4** ✅ Configure WebSocket client for backend connection (useWebSocket hook)
...
- **5.3.1** ✅ Implement message sending to Ego via API (POST /api/conversation/send)
- **5.3.2** ✅ Implement real-time conversation updates via WebSocket
- **5.3.3** ✅ Implement loop control buttons (start, pause, resume, stop)
- **5.3.4** ✅ Implement substrate file viewer (read-only access to all 12 file types)
...
- **6.1.1** ✅ Implement PLAN.md empty detector (via Id.detectIdle)
- **6.1.2** ✅ Implement PLAN.md completion detector (via Id.detectIdle)
- **6.1.3** ✅ Implement staleness timer (consecutiveIdleCycles in LoopOrchestrator)
- **6.1.4** ✅ Implement idle condition aggregator (IdleHandler.handleIdle)

### 6.2 Goal Generation Pipeline
*Depends on: 6.1.x completed*

- **6.2.1** ✅ Implement ID.md-driven goal candidate generator (via Id.generateDrives)
- **6.2.2** ✅ Implement goal template system (IdleHandler writes structured PLAN.md)
- **6.2.3** ✅ Implement goal priority scorer (GoalCandidate.priority field)
- **6.2.4** ✅ Implement Superego risk evaluation for generated goals (IdleHandler calls Superego.evaluateProposals)

### 6.3 Autonomous Plan Creation
*Depends on: 6.2.x completed*

- **6.3.1** ✅ Implement automatic PLAN.md generation from approved goals (IdleHandler.handleIdle)
- **6.3.2** ✅ Implement goal breakdown into actionable tasks (plan written with task checklist)
- **6.3.3** ✅ Implement automatic loop resume after plan creation (LoopOrchestrator resets idle counter)
...
- **7.1.1** ✅ Implement periodic evaluation trigger (every N cycles — superegoAuditInterval)
- **7.1.2** ✅ Implement event-based evaluation trigger (auditOnNextCycle flag + evaluation_requested event)
- **7.1.3** ✅ Implement user-requested evaluation trigger (POST /api/loop/audit)
- **7.1.4** ✅ Implement evaluation priority queue (auditOnNextCycle checked in runOneCycle)

### 7.2 Analysis Modules
*Depends on: 7.1.x completed*

- **7.2.1** ✅ Implement drift analyzer (DriftAnalyzer — compare structure vs. expected headings)
- **7.2.2** ✅ Implement consistency checker (ConsistencyChecker — cross-reference PLAN/SKILLS/MEMORY)
- **7.2.3** ✅ Implement security posture analyzer (SecurityAnalyzer — SECURITY.md compliance)
...
- **7.3.1** ✅ Implement governance report formatter (GovernanceReportStore)
- **7.3.2** ✅ Implement drift warning system (DriftAnalyzer score + findings)
- **7.3.3** ✅ Implement risk scoring system (drift score, security compliance)
- **7.3.4** ✅ Implement report storage in substrate/reports/ directory (GovernanceReportStore)
- **7.3.5** ✅ Implement API for governance reports (GET /api/reports, GET /api/reports/latest)
