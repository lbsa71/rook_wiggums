...
export async function startServer(config: AppConfig, options?: StartServerOptions): Promise<StartedServer> {
  ...
  const app = await createApplication({
    ...
    dualPrompt: config.dualPrompt,
    evaluateOutcome: config.evaluateOutcome,
    conversationArchive: config.conversationArchive,
    conversationIdleTimeoutMs: config.conversationIdleTimeoutMs,
    conversationSessionMaxDurationMs: config.conversationSessionMaxDurationMs,
    enableFileReadCache: config.enableFileReadCache,
    email: config.email,
    agora: config.agora,
    allowCommercialImplicitDispatch: config.allowCommercialImplicitDispatch,
